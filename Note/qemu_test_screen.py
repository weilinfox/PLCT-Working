...
class QemuVM(object):
    ...
    def screenExist(self):
        return os.system("screen -ls | grep "+self.name+" >/dev/null") == 0

    def screenWait(self):
        while self.screenExist():
            time.sleep(1)
...
if __name__ == "__main__":
    ...
    if os.system('for i in $(screen -ls | grep comss | sed "s/.*\(comss[0-9]*\).*/\\1/"); do screen -X -S $i quit; done') != 0:
        print("For loop strange exit")
        exit(-1)
