                ...
                if self.initTarget[2] > 1 and self.qemuVM.runArgs.find('multiMachine') != -1:
                    if self.initTarget[3] > 1 and self.qemuVM.runArgs.find('addNic') != -1:
                        ...
                        if tapnum > self.tapQueue.qsize():
                            ...
                            for i in range(1 , self.initTarget[2]):
                                ...
                                print(f"Wait vm {i-1} ready")
                                self.attachVM[i-1].waitReady()
                                print(f"Get vm {i-1} ready")
                    ...
                    if self.initTarget[3] > 1 and self.qemuVM.runArgs.find('addNic') != -1:
                        ...
                        if tapnum > self.tapQueue.qsize():
                            ...
                            print("Wait vm 11 ready")
                            self.qemuVM.waitReady()
                            print("Get vm 11 ready")
                            try:
                                self.qemuVM.runTest(self.initTarget[0])
                            except:
                                print("error "+self.initTarget[0])
...
class QemuVM(object):
    ...
    def start(self , disk=1 , machine=1 , tap_number=0 , taplist=[]):
        ...
        # x86_64
            ...
            "+kernelArg+" \
            "+biosArg+" \
            -initrd "+self.workingDir+"/initrd.img \
            -drive file="+drive+",format=qcow2,id=hd0,if=none \
            -object rng-random,filename=/dev/urandom,id=rng0 \
            -device virtio-rng,rng=rng0 \
            -device virtio-blk,drive=hd0 \
            -device qemu-xhci -usb -device usb-kbd -device usb-tablet -device usb-audio,audiodev=snd0 \
            -append 'root=/dev/vda2 rw console=ttyS0 swiotlb=1 loglevel=3 systemd.default_timeout_start_sec=600 selinux=0 highres=off mem="+str(memory_append)+"M earlycon' "
        ...
        if tap_number > 0:
            for i in range(tap_number-1):
                used_tap = taplist[i]
                cmd += "-netdev tap,id=net"+used_tap+",ifname="+used_tap+",script=no,downscript=no -device virtio-net-device,netdev=net"+used_tap+",mac=52:54:00:11:45:{:0>2d}".format(self.mac)+" "
                self.mac+=1
            if machine > 1:
                used_tap = taplist[-1]
                cmd += "-netdev tap,id=net"+used_tap+",ifname="+used_tap+",script=no,downscript=no -device virtio-net-device,netdev=net"+used_tap+",mac=52:54:00:11:45:{:0>2d}".format(self.mac)+" "
...
if __name__ == "__main__":
    ...
    if preImg == True or genList == True:
        ...
        if preImg == True:
            ...
            print(ssh_exec(preVM,'cd /root \n mkdir GitRepo \n cd GitRepo \n git clone https://kgithub.com/brsf11/mugen-riscv.git',timeout=600)[1])
